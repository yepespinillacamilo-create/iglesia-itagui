...
const T_EST_CLS = { pendiente:'bg-amber-50 text-amber-700', en_progreso:'bg-blue-50 text-blue-700', completada:'bg-emerald-50 text-emerald-700' }
const T_EST_LBL = { pendiente:'Pendiente', en_progreso:'En progreso', completada:'Completada' }
const P_CLS = { alta:'bg-red-50 text-red-600', media:'bg-orange-50 text-orange-600', baja:'bg-gray-100 text-gray-500' }
const R_EST_CLS = { programada:'bg-violet-50 text-violet-700', realizada:'bg-emerald-50 text-emerald-700', cancelada:'bg-gray-100 text-gray-500' }
const R_EST_LBL = { programada:'Programada', realizada:'Realizada', cancelada:'Cancelada' }
...
// ── SHARED UI (defined at module level — stable references) ───────────────────
...
// ── TASK CARD ─────────────────────────────────────────────────────────────────
function TaskCard({t, area, showArea, onEdit, onDelete, onStatus}) {
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex gap-1 flex-shrink-0">
              <button onClick={()=>onEdit(t)} className="p-1.5 text-gray-300 hover:text-gray-600 hover:bg-gray-100 rounded-lg"><Edit3 className="w-3.5 h-3.5"/></button>
              <button onClick={()=>onDelete(t.id)} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-3.5 h-3.5"/></button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            <Chip cls={T_EST_CLS[t.estado]}>{T_EST_LBL[t.estado]}</Chip>
            <Chip cls={P_CLS[t.prioridad]}>{t.prioridad}</Chip>
...
// ── MEETING CARD ──────────────────────────────────────────────────────────────
function MeetingCard({r, area, showArea, onEdit, onDelete, onStatus}) {
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex gap-1 flex-shrink-0">
              <button onClick={()=>onEdit(r)} className="p-1.5 text-gray-300 hover:text-gray-600 hover:bg-gray-100 rounded-lg"><Edit3 className="w-3.5 h-3.5"/></button>
              <button onClick={()=>onDelete(r.id)} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-3.5 h-3.5"/></button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            <Chip cls={R_EST_CLS[r.estado]}>{R_EST_LBL[r.estado]}</Chip>
...
// ── TASK MODAL (own state — no focus loss) ────────────────────────────────────
function TaskModal({open, onClose, areas, editing, saving, onSave}) {
  const EMPTY = {titulo:'',area_id:'',estado:'pendiente',prioridad:'media',responsable:'Camilo',fecha:'',notas:''}
  const [form, setForm] = useState(EMPTY)
  useEffect(() => {
    setForm(editing ? {...editing, area_id:String(editing.area_id), fecha:editing.fecha||''} : EMPTY)
  }, [open])
  if (!open) return null
  return (
    <ModalShell title={editing?'Editar tarea':'Nueva tarea'} onClose={onClose}>
      <Field label="Título *"><input value={form.titulo} onChange={e=>setForm({...form,titulo:e.target.value})} className={IC} placeholder="¿Qué hay que hacer?"/></Field>
      <Field label="Área *">
        <select value={form.area_id} onChange={e=>setForm({...form,area_id:e.target.value})} className={IC}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Estado">
          <select value={form.estado} onChange={e=>setForm({...form,estado:e.target.value})} className={IC}>
        ...
        <Field label="Prioridad">
          <select value={form.prioridad} onChange={e=>setForm({...form,prioridad:e.target.value})} className={IC}>
            <option value="alta">Alta</option><option value="media">Media</option><option value="baja">Baja</option>
          </select>
        </Field>
      </div>
      <Field label="Responsable">
        <select value={form.responsable} onChange={e=>setForm({...form,responsable:e.target.value})} className={IC}>
          {RESPONSABLES.map(r=><option key={r}>{r}</option>)}
        </select>
      </Field>
      <Field label="Fecha límite"><input type="date" value={form.fecha} onChange={e=>setForm({...form,fecha:e.target.value})} className={IC}/></Field>
      <Field label="Notas"><textarea value={form.notas} onChange={e=>setForm({...form,notas:e.target.value})} className={`${IC} resize-none`} rows={2} placeholder="Detalles adicionales..."/></Field>
      <SaveBtn label={editing?'Guardar cambios':'Crear tarea'} onClick={()=>onSave(form)} saving={saving}/>
    </ModalShell>
  )
}

// ── REUNION MODAL (own state) ─────────────────────────────────────────────────
function ReunionModal({open, onClose, areas, editing, saving, onSave}) {
  const EMPTY = {titulo:'',area_id:'',fecha:'',hora:'10:00',duracion:60,lugar:'',descripcion:'',responsable:'Camilo',estado:'programada'}
  const [form, setForm] = useState(EMPTY)
  useEffect(() => {
    setForm(editing ? {...editing, area_id:String(editing.area_id), hora:editing.hora?editing.hora.slice(0,5):'10:00'} : EMPTY)
  }, [open])
  if (!open) return null
  return (
    <ModalShell title={editing?'Editar reunión':'Nueva reunión'} onClose={onClose}>
      ...
      <Field label="Título *"><input value={form.titulo} onChange={e=>setForm({...form,titulo:e.target.value})} className={IC} placeholder="Ej: Reunión pastorado"/></Field>
      <Field label="Área *">
        <select value={form.area_id} onChange={e=>setForm({...form,area_id:e.target.value})} className={IC}>
          <option value="">Seleccionar área...</option>
          {areas.map(a=><option key={a.id} value={a.id}>{a.emoji} {a.nombre}</option>)}
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Fecha *"><input type="date" value={form.fecha} onChange={e=>setForm({...form,fecha:e.target.value})} className={IC}/></Field>
        <Field label="Hora"><input type="time" value={form.hora} onChange={e=>setForm({...form,hora:e.target.value})} className={IC}/></Field>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Duración">
          <select value={form.duracion} onChange={e=>setForm({...form,duracion:+e.target.value})} className={IC}>
      ...
      <Field label="Lugar"><input value={form.lugar} onChange={e=>setForm({...form,lugar:e.target.value})} className={IC} placeholder="Sala, Zoom, dirección..."/></Field>
      <Field label="Responsable">
        <select value={form.responsable} onChange={e=>setForm({...form,responsable:e.target.value})} className={IC}>
          {RESPONSABLES.map(r=><option key={r}>{r}</option>)}
        </select>
      </Field>
      <Field label="Descripción / Agenda"><textarea value={form.descripcion} onChange={e=>setForm({...form,descripcion:e.target.value})} className={`${IC} resize-none`} rows={2} placeholder="Puntos a tratar..."/></Field>
      <SaveBtn label={editing?'Guardar cambios':'Crear reunión'} onClick={()=>onSave(form)} saving={saving}/>
    </ModalShell>
  )
}
...
// ── AREA MODAL (own state) ────────────────────────────────────────────────────
function AreaModal({open, onClose, editing, saving, onSave}) {
  const [form, setForm] = useState({nombre:'',emoji:'📌',responsable:'Por definir',descripcion:''})
  useEffect(() => {
    setForm(editing ? {nombre:editing.nombre,emoji:editing.emoji,responsable:editing.responsable,descripcion:editing.descripcion||''} : {nombre:'',emoji:'📌',responsable:'Por definir',descripcion:''})
  }, [open])
  if (!open) return null
  return (
    <ModalShell title={editing?'Editar área':'Nueva área'} onClose={onClose}>
      <div className="grid grid-cols-4 gap-3">
        <Field label="Emoji"><input value={form.emoji} onChange={e=>setForm({...form,emoji:e.target.value})} className={IC} maxLength={2}/></Field>
        <div className="col-span-3"><Field label="Nombre *"><input value={form.nombre} onChange={e=>setForm({...form,nombre:e.target.value})} className={IC} placeholder="Nombre del área"/></Field></div>
      </div>
      <Field label="Responsable">
        <select value={form.responsable} onChange={e=>setForm({...form,responsable:e.target.value})} className={IC}>
          {RESPONSABLES.map(r=><option key={r}>{r}</option>)}
      ...
      <Field label="Descripción"><textarea value={form.descripcion} onChange={e=>setForm({...form,descripcion:e.target.value})} className={`${IC} resize-none`} rows={2} placeholder="¿De qué se encarga?"/></Field>
      <SaveBtn label={editing?'Guardar cambios':'Crear área'} onClick={()=>onSave(form)} saving={saving}/>
    </ModalShell>
  )
}

// ── GEST AREAS MODAL ──────────────────────────────────────────────────────────
function GestAreasModal({open, onClose, areas, onEdit, onDelete, onAdd}) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-3 sm:p-4">
      <div className="bg-white rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 sticky top-0 bg-white rounded-t-3xl">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-xl"><X className="w-4 h-4"/></button>
        </div>
        <div className="p-5">
              ...
              <div key={area.id} className="bg-gray-50 rounded-2xl p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-1">
                    <button onClick={()=>onEdit(area)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-white rounded-xl"><Edit3 className="w-3.5 h-3.5"/></button>
                    <button onClick={()=>setConfirmId(confirmId===area.id?null:area.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl"><Trash2 className="w-3.5 h-3.5"/></button>
                  </div>
                </div>
                {confirmId===area.id && (
                  <div className="mt-2 p-3 bg-red-50 rounded-xl">
                    ...
                    <div className="flex gap-2">
                      <button onClick={()=>{onDelete(area.id);setConfirmId(null)}} className="flex-1 bg-red-600 text-white text-xs py-1.5 rounded-lg font-medium">Sí, eliminar</button>
                      <button onClick={()=>setConfirmId(null)} className="flex-1 border border-gray-300 text-xs py-1.5 rounded-lg text-gray-600">Cancelar</button>
                    </div>
                  </div>
                )}
              </div>
...
// ── REUNIONES VIEW (own filter state) ────────────────────────────────────────
function ReunionesView({reuniones, areas, onEdit, onDelete, onStatus, onNew}) {
  const [busqueda, setBusqueda] = useState('')
  const [filtroEstado, setFiltroEstado] = useState('todos')
  const [filtroArea, setFiltroArea] = useState('todas')
  const getArea = id => areas.find(a=>a.id===id)
  const filtradas = [...reuniones].filter(r =>
    ...
    &&(filtroArea==='todas'||r.area_id===+filtroArea)
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-400 mb-4">{filtradas.length} reunión(es)</p>
      <div className="bg-violet-50 border border-violet-100 rounded-2xl p-3 mb-4 flex items-start gap-2">
        ...
        <p className="text-xs text-violet-700">Distintas a las tareas. Cada reunión tiene enlace para <strong>Google Calendar</strong>.</p>
      </div>
      <div className="space-y-3 mb-4">
        <div className="relative">
          <Search className="w-4 h-4 text-gray-300 absolute left-3 top-1/2 -translate-y-1/2"/>
          <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar reuniones..."
            className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-gray-400 bg-white"/>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <select value={filtroEstado} onChange={e=>setFiltroEstado(e.target.value)} className={IC}>
            <option value="todos">Estado: Todos</option><option value="programada">Programada</option><option value="realizada">Realizada</option><option value="cancelada">Cancelada</option>
          </select>
          <select value={filtroArea} onChange={e=>setFiltroArea(e.target.value)} className={IC}>
            <option value="todas">Área: Todas</option>
            {areas.map(a=><option key={a.id} value={a.id}>{a.emoji} {a.nombre}</option>)}
          </select>
        </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:'#F8F7F4'}}>
      <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-sm border border-gray-100">
        ...
        {error && <div className="bg-red-50 border border-red-100 text-red-600 text-sm p-3 rounded-xl mb-4 text-center">{error}</div>}
        <div className="space-y-3">
          <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Correo electrónico" className={IC} onKeyDown={e=>e.key==='Enter'&&handleLogin()}/>
          <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Contraseña" className={IC} onKeyDown={e=>e.key==='Enter'&&handleLogin()}/>
...
function MainApp({user}) {
  ...
  const [mGest, setMGest] = useState(false)
  ...
  const loadAll = useCallback(async()=>{ setLoading(true); await Promise.all([loadAreas(),loadTareas(),loadReuniones(),loadBitacora()]); setLoading(false) },[loadAreas,loadTareas,loadReuniones,loadBitacora])
  ...
  const handleSaveTask = async form => {
    ...
    setSaving(true)
    const p = {titulo:form.titulo,area_id:+form.area_id,estado:form.estado,prioridad:form.prioridad,responsable:form.responsable,fecha:form.fecha||null,notas:form.notas}
    if (editTarea) { await supabase.from('tareas').update(p).eq('id',editTarea.id); showToast('Tarea actualizada') }
    else { await supabase.from('tareas').insert(p); showToast('Tarea creada ✓') }
    await loadTareas(); setMTarea(false); setEditTarea(null); setSaving(false)
  }
  const handleDeleteTask = async id => { await supabase.from('tareas').delete().eq('id',id); await loadTareas(); showToast('Eliminada','del') }
  const handleStatus = async(id,estado) => { setTareas(ts=>ts.map(t=>t.id===id?{...t,estado}:t)); await supabase.from('tareas').update({estado}).eq('id',id) }

  const handleSaveReunion = async form => {
    if (!form.titulo.trim()||!form.area_id||!form.fecha) return
    setSaving(true)
    const p = {titulo:form.titulo,area_id:+form.area_id,fecha:form.fecha,hora:form.hora,duracion:+form.duracion,lugar:form.lugar,descripcion:form.descripcion,responsable:form.responsable,estado:form.estado}
    if (editReunion) { await supabase.from('reuniones').update(p).eq('id',editReunion.id); showToast('Reunión actualizada') }
    else { await supabase.from('reuniones').insert(p); showToast('Reunión creada ✓') }
    await loadReuniones(); setMReunion(false); setEditReunion(null); setSaving(false)
  }
  const handleDeleteReunion = async id => { await supabase.from('reuniones').delete().eq('id',id); await loadReuniones(); showToast('Eliminada','del') }
  const handleRStatus = async(id,estado) => { setReuniones(rs=>rs.map(r=>r.id===id?{...r,estado}:r)); await supabase.from('reuniones').update({estado}).eq('id',id) }
  ...
  const handleSaveArea = async form => {
    ...
    setSaving(true)
    if (editArea) { await supabase.from('areas').update(form).eq('id',editArea.id); showToast('Área actualizada') }
    else { await supabase.from('areas').insert(form); showToast('Área creada ✓') }
    await loadAreas(); setMArea(false); setEditArea(null); setSaving(false)
  }
  const handleDeleteArea = async id => {
    await supabase.from('areas').delete().eq('id',id)
    await loadAll(); showToast('Área eliminada','del')
  }

  const handleLogout = () => supabase.auth.signOut()

  const nav4 = [{id:'dashboard',label:'Inicio',icon:LayoutDashboard},{id:'areas',label:'Áreas',icon:Layers},{id:'tareas',label:'Tareas',icon:CheckSquare},{id:'reuniones',label:'Reuniones',icon:Calendar}]
  const isAct = id => vista===id||(id==='areas'&&vista==='area_detalle')

  // ── VIEWS ────────────────────────────────────────────────
  const stats = { pendiente:tareas.filter(t=>t.estado==='pendiente').length, en_progreso:tareas.filter(t=>t.estado==='en_progreso').length, completada:tareas.filter(t=>t.estado==='completada').length }
  const proxReus = [...reuniones].filter(r=>r.estado==='programada'&&r.fecha>=HOY).sort((a,b)=>(`${a.fecha}${a.hora}`>`${b.fecha}${b.hora}`)?1:-1).slice(0,3)
    ...
    <div className="min-h-screen flex" style={{background:'#F8F7F4'}}>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0"><p className="text-xs font-semibold text-gray-700">Camilo y Karen</p><p className="text-xs text-gray-400 truncate">{user.email}</p></div>
            </div>
            <button onClick={handleLogout} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-xl flex-shrink-0"><LogOut className="w-4 h-4"/></button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 md:ml-56 pb-24 md:pb-10 min-h-screen">
        <div className="max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {vista==='dashboard' && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">Iglesia Itagüí · {new Date().toLocaleDateString('es-CO',{weekday:'long',day:'numeric',month:'long'})}</p>
              </div>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {[{l:'Pendientes',v:stats.pendiente,fg:'text-amber-700',bg:'#FFFBEB'},{l:'En progreso',v:stats.en_progreso,fg:'text-blue-700',bg:'#EFF6FF'},{l:'Urgentes',v:urgCnt,fg:'text-red-700',bg:'#FEF2F2'},{l:'Reuniones',v:rProgCnt,fg:'text-violet-700',bg:'#F5F3FF'}].map(s=>(
              ...
              {urgCnt>0&&(
                <div className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-2xl p-4">
                  ...
                  <div className="flex-1"><p className="text-sm font-semibold text-red-700">{urgCnt} tarea{urgCnt>1?'s':''} urgente{urgCnt>1?'s':''}</p><p className="text-xs text-red-400 mt-0.5">Alta prioridad · sin completar</p></div>
                  <button onClick={()=>setVista('tareas')} className="text-xs bg-red-600 text-white px-3 py-1.5 rounded-xl hover:bg-red-700 flex-shrink-0">Ver</button>
                </div>
              )}
              {proxReus.length>0&&(
                <div>
                  <div className="flex items-center justify-between mb-3"><p className="text-sm font-semibold text-gray-700">Próximas reuniones</p><button onClick={()=>setVista('reuniones')} className="text-xs text-violet-500">Ver todas →</button></div>
                  <div className="space-y-2">
                    {proxReus.map(r=>{const area=getArea(r.area_id);return(
                      <div key={r.id} className="bg-white border border-gray-100 rounded-2xl p-4 flex items-center gap-3">
                        <div className="w-10 h-10 bg-violet-50 rounded-xl flex items-center justify-center flex-shrink-0"><span className="text-xl">{area?.emoji}</span></div>
                        <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{r.titulo}</p><p className="text-xs text-gray-400 mt-0.5">{r.fecha}{r.hora?` · ${r.hora.slice(0,5)}`:''}{r.lugar?` · ${r.lugar}`:''}</p></div>
                        <a href={gcalUrl(r)} target="_blank" rel="noopener noreferrer" className="p-2 text-violet-400 hover:text-violet-600 hover:bg-violet-50 rounded-xl flex-shrink-0"><ExternalLink className="w-4 h-4"/></a>
                      </div>
                    )})}
                  </div>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3"><p className="text-sm font-semibold text-gray-700">Tareas urgentes</p><button onClick={()=>setVista('tareas')} className="text-xs text-blue-500">Ver todas →</button></div>
                {tareas.filter(isUrgente).length===0
                  ...
                  :<div className="space-y-2">{tareas.filter(isUrgente).slice(0,4).map(t=>{const area=getArea(t.area_id);return(
                    <div key={t.id} className="bg-white rounded-2xl p-4 flex items-center gap-3 border border-gray-100">
                      ...
                      <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{t.titulo}</p><p className="text-xs text-gray-400 mt-0.5">{area?.nombre}{t.fecha?` · ${t.fecha}`:''}</p></div>
                      <Chip cls={T_EST_CLS[t.estado]}>{T_EST_LBL[t.estado]}</Chip>
                    </div>
                  )})}</div>}
              </div>
              <div>
                <div className="flex items-center justify-between mb-3"><p className="text-sm font-semibold text-gray-700">Áreas</p><button onClick={()=>setVista('areas')} className="text-xs text-blue-500">Ver todas →</button></div>
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {areas.map(area=>{const cnt=tareas.filter(t=>t.area_id===area.id&&t.estado!=='completada').length;return(
                    <button key={area.id} onClick={()=>{setAreaActual(area);setVista('area_detalle')}} className="bg-white border border-gray-100 rounded-2xl p-3 text-center hover:border-gray-300 transition-all">
          ...
          {vista==='areas' && (
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={()=>setMGest(true)} className="flex items-center gap-1.5 text-sm border border-gray-200 px-3 py-2 rounded-xl hover:bg-gray-50 text-gray-600"><Settings className="w-4 h-4"/> Gestionar</button>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {areas.map(area=>{
                  ...
                  return(
                    <button key={area.id} onClick={()=>{setAreaActual(area);setVista('area_detalle')}} className="bg-white border border-gray-100 rounded-2xl p-5 text-left hover:border-gray-300 hover:shadow-sm transition-all">
                      <div className="flex items-start justify-between mb-3"><span className="text-3xl">{area.emoji}</span><ChevronRight className="w-4 h-4 text-gray-300 mt-1"/></div>
          ...
          {/* TAREAS */}
          {vista==='tareas' && (
            <TareasView tareas={tareas} areas={areas}
              onEdit={t=>{setEditTarea(t);setMTarea(true)}}
              onDelete={handleDeleteTask}
              onStatus={handleStatus}
              onNew={()=>{setEditTarea(null);setMTarea(true)}}/>
          )}

          {/* REUNIONES */}
          {vista==='reuniones' && (
            <ReunionesView reuniones={reuniones} areas={areas}
              onEdit={r=>{setEditReunion(r);setMReunion(true)}}
              onDelete={handleDeleteReunion}
              onStatus={handleRStatus}
              onNew={()=>{setEditReunion(null);setMReunion(true)}}/>
          )}
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-100 z-20" style={{paddingBottom:'env(safe-area-inset-bottom)'}}>
        <div className="flex justify-around px-2 py-2">
          {nav4.map(({id,label,icon:Icon})=>{
            const act=isAct(id)
            return(
              <button key={id} onClick={()=>setVista(id)} className={`relative flex flex-col items-center gap-0.5 py-1.5 px-3 rounded-xl transition-all ${act?'text-gray-900':'text-gray-400'}`}>
      ...
      {/* MODALS */}
      <TaskModal open={mTarea} onClose={()=>{setMTarea(false);setEditTarea(null)}} areas={areas} editing={editTarea} saving={saving} onSave={handleSaveTask}/>
      <ReunionModal open={mReunion} onClose={()=>{setMReunion(false);setEditReunion(null)}} areas={areas} editing={editReunion} saving={saving} onSave={handleSaveReunion}/>
      <BitacoraModal open={mBitacora} onClose={()=>setMBitacora(false)} saving={saving} onSave={handleSaveBit}/>
      <AreaModal open={mArea} onClose={()=>{setMArea(false);setEditArea(null)}} editing={editArea} saving={saving} onSave={handleSaveArea}/>
      <GestAreasModal open={mGest} onClose={()=>setMGest(false)} areas={areas}
        onEdit={a=>{setEditArea(a);setMArea(true);setMGest(false)}}
        onDelete={handleDeleteArea}
        onAdd={()=>{setEditArea(null);setMArea(true);setMGest(false)}}/>

      {/* TOAST */}
      {toast&&(
        <div className={`fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-2.5 rounded-2xl text-sm font-medium shadow-xl whitespace-nowrap ${toast.type==='del'?'bg-red-600 text-white':'bg-gray-900 text-white'}`}>
          {toast.msg}
        </div>
      )}
    </div>
...
export default function App() {
  ...
  if (session===undefined) return <div className="min-h-screen flex items-center justify-center" style={{background:'#F8F7F4'}}><div className="text-center"><span className="text-5xl block mb-4">🏛️</span><Loader2 className="w-6 h-6 animate-spin text-gray-400 mx-auto"/></div></div>
